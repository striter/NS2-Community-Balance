...
local function PerformAttackEntity( eyePos, target, lastSeenPos, bot, brain, move )
    ...
    ----------------------------------------------------------------------
    -- Eier niemals beschie遝n � Exo soll sie nur zertreten
    ----------------------------------------------------------------------
    ...
    ----------------------------------------------------------------------
    -- Dynamische Distanz: Wenn Eier in der N鋒e sind ? mehr Bewegungsfreiheit
    ----------------------------------------------------------------------
    ...
    ----------------------------------------------------------------------
    -- Wenn kein klarer Schuss m鰃lich ? weiterlaufen (aber NICHT wenn Ei-Commitment)
    ----------------------------------------------------------------------
    if not hasClearShot and not committedToEgg then
        ...
        elseif dist > dynamicCloseDist then
            ...
            -- NEU: Egg-Commitment 黚erschreibt Strafen
        ...
        ------------------------------------------------------------------
        -- Aiming abh鋘gig von Railgun/Minigun
        ------------------------------------------------------------------
...
local function PerformAttack(eyePos, mem, bot, brain, move)
    ...
    if target ~= nil then
        ...
        if isSelfBot and target:GetTeamNumber() ~= bot:GetTeamNumber() then
            ...
            -- Letzte Meldung f黵 diese Location (geteilt von ALLEN Marines/Exos)
...
function(bot, brain, exo)
    ...
    for _, arc in ipairs(arcs) do
        if IsValid(arc) and arc:GetIsAlive() then
            ...
            -- ? ARC wird verteidigt ? 黚erspringen
            if ArcIsDefended(arc) then
                goto continue_arc
            end
