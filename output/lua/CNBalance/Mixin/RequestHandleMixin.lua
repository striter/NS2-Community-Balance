...
if Server then
    ...
    function RequestHandleMixin:NanoSelf()
        local time = Shared.GetTime()
        if time < self.timeLastPrimaryRequestHandle then return end

        if not HasMixin(self, "NanoShieldAble") then return end
        if not self:GetCanBeNanoShielded() then return end
        if not self:GetIsInCombat() then return end
        
        if self:GetResources() < kAutoNanoShieldPRes then return end

        self.timeLastPrimaryRequestHandle = time + kAutoNanoShieldCooldown
        self:AddResources(-kAutoNanoShieldPRes)
    ...
    function RequestHandleMixin:HandleManualAlert(techId)
        ...
        if techId == kTechId.MarineAlertNeedMedpack then
            if self.GetIsBMAC and self:GetIsBMAC() then
                if GetIsTechAvailable(self:GetTeamNumber(),kTechId.NanoShield) then
                    self:AddTimedCallback(self.NanoSelf,kAlertHandleDelay)
                end
                return true
            else
