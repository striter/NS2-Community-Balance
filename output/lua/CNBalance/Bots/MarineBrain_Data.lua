...
kMarineBrainObjectiveActions =
{
    ...
    function(bot, brain, marine)
        ...
        if not techTree:GetIsTechAvailable(kTechId.DualMinigunExosuit, true) then
            return kNilAction
        end
        ...
        if not hasBoughtDesiredWeapon and brain.activeWeaponPurchaseTechId ~= nil then
            ...
            if techTree then
                for _, weaponTechId in ipairs(weapons) do
                    if techTree:GetIsTechAvailable(weaponTechId) then
    ...
    function(bot, brain, marine)    --FIXME This should check for JPs on ground FIRST, then fail-over to purchase
        ...
        if proto and protoDist and not marine:isa("JetpackMarine") and techTree:GetIsTechAvailable(kTechId.Jetpack) and resources >= LookupTechData(kTechId.Jetpack, kTechDataCostKey) then
...
local function EvalActiveUrgenciesTable(numOthers)
    local activeUrgencies =
    {
        ...
        [kMinimapBlipType.Prowler] =       numOthers >= 2 and 0.1 or 5.0,
