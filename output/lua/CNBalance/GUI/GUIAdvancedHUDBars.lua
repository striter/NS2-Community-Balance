...
function GUIAdvancedHUDBars:Update(deltaTime)
	...
	if player and player:GetIsAlive() and self.vis then
		...
		local health = player:GetIgnoreHealth() and 0 or math.max(1, math.floor(player:GetHealth()))
		-- Do not multiply by kHealthPointsPerArmor here so we can display the armor number directly later
		local armor = player:GetIgnoreHealth() and math.max(1, math.floor(player:GetArmor())) or math.floor(player:GetArmor())
		...
		if (player:isa("Marine") or player:isa("Exo") or player:isa("Alien")) and hudbars == 1 then
			if self.lastHealth ~= health or self.lastArmor ~= armor then
				if not player:GetIgnoreHealth() then
		...
		local armorColor = ConditionalValue(player:GetIgnoreHealth() and armor < 100, pulsatingRed, hudbars == 1 and kArmorColors[teamIndex] or kWhite)
