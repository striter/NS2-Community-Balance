...
function GUIHiveStatus:Initialize()
    ...
    local kContainerOffset = 10
    self.kHiveContainerPositions = {}
    self.kHiveContainerPositions[1] = GUIScale( Vector( 32, 5 + kContainerOffset , 0 ) )
    self.kHiveContainerPositions[2] = GUIScale( Vector( 20, 90 + kContainerOffset, 0 ) )
    self.kHiveContainerPositions[3] = GUIScale( Vector( 6, 175 + kContainerOffset , 0 ) )
    self.kHiveContainerPositions[4] = GUIScale( Vector( 10, 260 + kContainerOffset , 0 ) )
    self.kHiveContainerPositions[5] = GUIScale( Vector( 20, 345 + kContainerOffset, 0 ) )
    ...
    local kGap = 42
    local yOffset = -25
    self.kLifeformCountTextPosition = Vector( 12.1, 8.4, 0 )
    self.teamSkulkTable = self:InitializeLifeformCount( Vector( kStart, yOffset, 0  ),5)
    self.teamProwlerTable = self:InitializeLifeformCount( Vector( kStart + kGap * 1, yOffset, 0  ),6)
...
function GUIHiveStatus:InitializeLifeformCount(position, iconIndex)
    local lifeformIcon = GUIManager:CreateGraphicItem()
    lifeformIcon:SetSize(self.kCommanderIconSize)
    lifeformIcon:SetPosition(position)
    lifeformIcon:SetAnchor( GUIItem.Left, GUIItem.Top )
    ...
    lifeformCount:SetAnchor( GUIItem.Left, GUIItem.Top )
    lifeformCount:SetFontName( Fonts.kAgencyFB_Large_Bold )
    lifeformCount:SetColor( GUIHiveStatus.kEggsCountFontColor )
    lifeformCount:SetScale( self.kEggTextFontScale )  --Scaled???
    lifeformCount:SetLayer( kGUILayerPlayerHUDForeground2 )
...
function GUIHiveStatus:Uninitialize()
    ...
    self:UnitializeLifeformCount(self.teamSkulkTable)
    self:UnitializeLifeformCount(self.teamGorgeTable)
    self:UnitializeLifeformCount(self.teamLerkTable)
...
function GUIHiveStatus:UnitializeLifeformCount(table)
    if not table then return end
    GUI.DestroyItem(table.icon)
    GUI.DestroyItem(table.text)
    table.icon = nil
    table.text = nil
end
...
function GUIHiveStatus:UpdateCommanderIcon( slotIdx )
    ...
    if self.statusSlots[slotIdx]._locationId == commanderLocationId then
        ...
        if commanderType > 0 then
            ...
            texCoords = {( commanderType - 1)* 72 ,0 ,commanderType * 72 ,68 }
        end
